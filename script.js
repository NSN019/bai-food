...
function getRecoveryAccessToken(){const hash=new URLSearchParams(location.hash.replace(/^#/,""));return hash.get("type")==="recovery"?hash.get("access_token"):null}
function initPasswordRecovery(){if(typeof location==="undefined")return;const token=getRecoveryAccessToken();if(!token)return;window.baiFoodRecoveryToken=token;showModal("recovery-modal")}
async function saveRecoveredPassword(){const password=document.getElementById("recovery-password").value,repeat=document.getElementById("recovery-password-repeat").value,error=document.getElementById("recovery-error"),button=document.getElementById("recovery-save-button");error.style.display="none";if(password.length<8){error.textContent="Пароль должен содержать минимум 8 символов.";error.style.display="block";return}if(password!==repeat){error.textContent="Пароли не совпадают.";error.style.display="block";return}button.disabled=true;button.textContent="Сохраняем…";try{const response=await fetch(`${SUPABASE_URL}/auth/v1/user`,{method:"PUT",headers:{"Content-Type":"application/json",apikey:SUPABASE_ANON_KEY,Authorization:`Bearer ${window.baiFoodRecoveryToken}`},body:JSON.stringify({password})});if(!response.ok)throw new Error(await response.text());history.replaceState(null,"",location.pathname+location.search);hideModal("recovery-modal");alert("Пароль успешно изменён. Теперь войдите в кассу с новым паролем.")}catch(e){console.error(e);error.textContent="Не удалось изменить пароль. Запросите новую ссылку восстановления.";error.style.display="block"}finally{button.disabled=false;button.textContent="Сохранить пароль"}}
