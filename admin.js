...
function normalizeOrder(order){
  ...
  const deliveryFee=order.delivery_fee!=null?Number(order.delivery_fee):(Number(deliveryItem?.price)||0);
  const serviceFee=order.service_fee!=null?Number(order.service_fee):(order.order_type==="dine_in"?Math.max(0,(Number(order.total)||0)-itemSubtotal):0);
...
function renderOrders(){
  ...
  filtered.forEach(order=>{
    ...
    const deliveryHTML=isDelivery?`<div class="delivery-breakdown"><div><span>Товары</span><strong>${formatMoney(order.itemSubtotal)}</strong></div><div><span>Доставка</span><strong>${order.deliveryFee===0?"Бесплатно":formatMoney(order.deliveryFee)}</strong></div><div><span>Итого</span><strong>${formatMoney(order.total)}</strong></div></div>`:`<div class="delivery-breakdown"><div><span>Товары</span><strong>${formatMoney(order.itemSubtotal)}</strong></div><div><span>Обслуживание 10%</span><strong>${formatMoney(order.serviceFee)}</strong></div><div><span>Итого</span><strong>${formatMoney(order.total)}</strong></div></div>`;
